...
bool ResourceManager::loadStage(
  ...
  bool buildCollisionMesh =
      ((_physicsManager != nullptr) &&
       (_physicsManager->getInitializationAttributes()->getSimulator() !=
        "none"));
  const bool needsCollisionMeshForNavmesh =
      (metadataMediator_->getSimulatorConfiguration().navMeshSettings !=
       nullptr) &&
      (stageAttributes->getRenderAssetType() ==
       AssetType::GaussianSplatting) &&
      (!stageAttributes->getCollisionAssetFullPath().empty()) &&
      (stageAttributes->getCollisionAssetFullPath() != EMPTY_SCENE);
  const bool loadCollisionInfo =
      buildCollisionMesh || needsCollisionMeshForNavmesh;
  const std::string renderLightSetupKey(stageAttributes->getLightSetupKey());
  std::map<std::string, AssetInfo> assetInfoMap =
      createStageAssetInfosFromAttributes(stageAttributes, loadCollisionInfo,
                                          createSemanticMesh);
  ...
  renderCreation.normalizeGaussianQuaternion = stageAttributes->getGaussianNormalizeQuaternion();
  ...
  if ((_physicsManager != nullptr) && (infoToUse.filepath != EMPTY_SCENE)) {
    const bool shouldBuildMeshGroups =
        (infoToUse.type != AssetType::GaussianSplatting);
    if (shouldBuildMeshGroups) {
...
bool ResourceManager::buildMeshGroups(
  ...
  if (colMeshGroupIter == collisionMeshGroups_.end()) {
    AssetInfo collisionInfoToUse = info;
    std::string collisionFilepath = info.filepath;

    //! Collect collision mesh group
    bool colMeshGroupSuccess = false;
    if ((collisionInfoToUse.type == AssetType::InstanceMesh) &&
        !collisionInfoToUse.hasSemanticTextures) {
      // PLY Semantic mesh
      colMeshGroupSuccess =
          buildStageCollisionMeshGroup<GenericSemanticMeshData>(
              collisionFilepath, meshGroup);
    } else if ((collisionInfoToUse.type == AssetType::Mp3dMesh ||
                collisionInfoToUse.type == AssetType::Unknown) ||
               ((collisionInfoToUse.type == AssetType::InstanceMesh) &&
                collisionInfoToUse.hasSemanticTextures)) {
      // GLB Mesh
      colMeshGroupSuccess =
          buildStageCollisionMeshGroup<GenericMeshData>(collisionFilepath,
                                                        meshGroup);
    }
...
bool ResourceManager::loadRenderAsset(const AssetInfo& info) {
  ...
  if (!fileAssetIsLoaded) {
    ...
    if (meshSuccess) {
      ...
      if (info.type != AssetType::Primitive) {
        std::vector<CollisionMeshData> meshGroup;
        // For Gaussian Splatting render assets, collision should be specified
        // separately via stage 'collision_asset'; do not auto-build here.
        if (info.type != AssetType::GaussianSplatting) {
...
scene::SceneNode* ResourceManager::createRenderAssetInstance(
  ...
  if (info.type == AssetType::InstanceMesh) {
    ...
    newNode = createRenderAssetInstanceGaussianSplatting(
        creation, parent, drawables);
  } else if (isRenderAssetGeneral(info.type) ||
...
// Removed: buildGaussianCollisionMeshGroup; collision should come from explicit
// collision assets defined in the Stage configuration.
...
bool ResourceManager::loadRenderAssetGaussianSplatting(const AssetInfo& info) {
  ...
  const auto& fgSet = gsImporter->getForegroundData();
  const auto& bgSet = gsImporter->getBackgroundData();
  const bool hasForeground = fgSet.count > 0;
  const bool hasBackground = bgSet.count > 0;
  size_t gaussianCount = gsImporter->getGaussianCount();
  ESP_DEBUG() << "Loaded" << gaussianCount
              << "Gaussians from Gaussian Splatting asset:" << filename;

  gaussianMetadataCache_[filename] = GaussianMetadata{
      gsImporter->is4D(),
      gsImporter->isHybrid(),
      gsImporter->getDuration(),
      gsImporter->getInterval(),
      gsImporter->getForegroundInterval()};

  // Create GaussianSplattingData and populate it
  auto gaussianData = std::make_unique<GaussianSplattingData>();
  GaussianSplattingData::Layout layout = GaussianSplattingData::Layout::k3D;
  if (hasForeground && hasBackground) {
    layout = GaussianSplattingData::Layout::kHybrid;
  } else if (hasForeground) {
    layout = GaussianSplattingData::Layout::k4D;
  }
  gaussianData->setLayout(layout);

  // Build bounding box
  Magnum::Range3D boundingBox;
  bool firstPoint = true;
  auto updateBoundingBox = [&](const Mn::Vector3& pos) {
    if (firstPoint) {
      boundingBox = Magnum::Range3D{pos, pos};
      firstPoint = false;
    } else {
      boundingBox =
          Magnum::Math::join(boundingBox, Magnum::Range3D{pos, pos});
    }
  };
  ...
  auto addStaticSet = [&](const GaussianSplattingImporter::GaussianSetData& set) {
    for (size_t i = 0; i < set.count; ++i) {
      GaussianSplat splat;
      splat.position =
          i < set.positions.size() ? set.positions[i] : Mn::Vector3{0.0f};
      splat.normal =
          i < set.normals.size() ? set.normals[i] : Mn::Vector3{0.0f};
      splat.f_dc = i < set.sh_dc.size() ? set.sh_dc[i] : Mn::Vector3{0.0f};
      splat.opacity =
          i < set.opacities.size() ? set.opacities[i] : 1.0f;
      splat.scale =
          i < set.scales.size() ? set.scales[i] : Mn::Vector3{0.0f};
      splat.rotation =
          i < set.rotations.size()
              ? set.rotations[i]
              : Mn::Quaternion{Mn::Vector3{0.0f}, 1.0f};
      if (set.hasRotationR && i < set.rotationsR.size()) {
  ...
  auto addDynamicSet = [&](const GaussianSplattingImporter::GaussianSetData& set) {
    for (size_t i = 0; i < set.count; ++i) {
      GaussianSplat4D splat;
      splat.position =
          i < set.positions.size() ? set.positions[i] : Mn::Vector3{0.0f};
      splat.normal =
          i < set.normals.size() ? set.normals[i] : Mn::Vector3{0.0f};
      ...
      if (i < set.times.size()) {
        splat.time = set.times[i];
      }
      if (i < set.timeScales.size()) {
        splat.timeScale = set.timeScales[i];
      }
      if (i < set.motions.size()) {
        splat.motion = set.motions[i];
      }
      if (i < set.motionCoeffs.size()) {
        if (set.motionStride >= 6) {
          splat.motionAccel = Mn::Vector3{set.motionCoeffs[i][3],
                                          set.motionCoeffs[i][4],
                                          set.motionCoeffs[i][5]};
        }
        if (set.motionStride >= 9) {
          splat.motionJerk = Mn::Vector3{set.motionCoeffs[i][6],
                                         set.motionCoeffs[i][7],
                                         set.motionCoeffs[i][8]};
        }
      }
  ...
  resourceDict_.emplace(filename, std::move(loadedAssetData));
...
Corrade::Containers::Optional<ResourceManager::GaussianMetadata>
ResourceManager::getGaussianMetadata(const std::string& filepath) const {
  auto it = gaussianMetadataCache_.find(filepath);
  if (it != gaussianMetadataCache_.end()) {
    return it->second;
  }
...
scene::SceneNode* ResourceManager::createRenderAssetInstanceGaussianSplatting(
  ...
  // Create a new scene node for the Gaussian Splatting asset
  ...
  int meshIDLocal = meshMetaData.root.meshIDLocal;
  
  // Get the Gaussian data from meshes_
  auto meshIter = meshes_.find(meshMetaData.meshIndex.first + meshIDLocal);
  CORRADE_INTERNAL_ASSERT(meshIter != meshes_.end());
  
  auto* gaussianData = dynamic_cast<GaussianSplattingData*>(meshIter->second.get());
  CORRADE_INTERNAL_ASSERT(gaussianData);

#ifdef ESP_BUILD_WITH_CUDA
  // Create GaussianSplattingDrawable
  // Convert raw pointer to shared_ptr for the drawable
  // Note: We need to ensure the GaussianData outlives the drawable
  // The resourceDict_ owns the data, and drawables should be destroyed first
  std::shared_ptr<GaussianSplattingData> sharedGaussianData(
      gaussianData, [](GaussianSplattingData*) {
        // Empty deleter - ownership remains with meshes_ map
      });
  
  // Create drawable configuration with minimal setup for Gaussian Splatting
  gfx::DrawableConfiguration config(
      Mn::ResourceKey{},  // No lighting setup needed
      Mn::ResourceKey{},  // No material key needed
      metadata::attributes::ObjectInstanceShaderType::Unspecified,  // No shader type needed
      drawables,  // drawable group
      nullptr,  // No skin data
      nullptr,  // No PBR IBL data
      nullptr   // No PBR shader config
  );
  
  new gfx::GaussianSplattingDrawable(newNode, sharedGaussianData, config, creation.normalizeGaussianQuaternion);
  
  ESP_DEBUG() << "Created GaussianSplattingDrawable instance for"
              << creation.filepath << "with"
              << gaussianData->getGaussianCount() << "Gaussians";
#else
  ESP_WARNING() << "CUDA support required for Gaussian Splatting rendering."
                << "Asset" << creation.filepath << "loaded but cannot be rendered.";
#endif
